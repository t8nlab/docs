---
title: 2. Actions
description: Execute request-aware business logic in JavaScript, TypeScript (Beta), or Rust, compiled and embedded into TitanPl’s native server.
---
...
### 📁 Nested Route Organization & Recursive Discovery

You can organize your actions into subfolders to keep your codebase clean and structured:

```js title="app.js"
t.post("/auth/login").action("auth/login")
```

The bundler recursively scans `app/actions/` and bundles each action into its exact nested location under `dist/actions/` (e.g., compiling `app/actions/auth/login.js` into `dist/actions/auth/login.jsbundle`).

### 🧼 Clean Slashes Mapping

Route definitions automatically normalize paths to prevent registration failures. Both options below are matched to `"auth/login"`, aligning trailing, internal, and back/forward slashes with the compiled action bundle paths:

```js title="app.js"
t.post("/auth/login").action("auth/login.js") // Auto-normalized to "auth/login"
t.post("/auth/register").action("auth/register") // Normalizes trailing and internal slashes
```

<Callout title="Naming Convention Warning" type="warn">
  You **MUST** ensure the following match:
  1. The relative **file path and name** in `app/actions` (e.g., `auth/login.js`)
  2. The **route reference** in `app.js` (e.g., `.action("auth/login")`)
  
  Titan uses this convention to automatically wire your application. If the resolved name does not match, the action will not be found.
</Callout>
...
Actions live in the `app/actions` directory and can be nested arbitrarily.
